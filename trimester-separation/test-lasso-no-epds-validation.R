...
make.confusion.row <- function(prediction_response, actual, cutoff) {
  ...
  # added 2022-11-11
  dat %>% dt_del_cols("PHQ_Dep", "EPDS_Dep")
  ...
  validation_set <- DT[for_validation]
  ...
  validation_set %>% dt_del_cols("MyLua_Index_PatientID", "Depression")

  DT %>% fwrite("~/Desktop/test-in-python/DT.csv")
  validation_set %>% fwrite("~/Desktop/test-in-python/validation_set.csv")
  ...
  X <- model.matrix(deptarget ~ ., data=validation_set)[, -1]
  Y <- as.matrix(validation_set[, 1]+0)
  ...
  list(rres=rres, cres=cres, cms=cms, every_patient=every_patient) -> THIS
}
